...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const { filePath, searchProperty, searchValue } = request.params.arguments as {
    filePath: string;
  ...
  try {
    ...
    const allTargetNodes: { node: any, path: string }[] = [];
    
    function searchNodes(obj: any, currentPath = '') {
    ...
    const pathPatterns: { [pattern: string]: any[] } = {};
    ...
    if (!searchValue) {
      const valueFrequency: { [value: string]: number } = {};
      allTargetNodes.forEach(item => {
    ...
    mainReport += `=========================\n\n`;
    const relatedProperties = new Set<string>();
    const relatedPropertyValues: { [prop: string]: Set<any> } = {};
    allTargetNodes.forEach(item => {
...
"SomeMapper": {
  ...
  "Property": "THE_CONTAINING_PROPERTY" // Replace with actual containing property
    ...
    const toolResponse: { type: "text"; text: string }[] = [{
      type: "text",
      text: mainReport
    }];
